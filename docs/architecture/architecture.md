...
**详细设计文档**：
- [微服务架构设计与通信机制](microservice_architecture.md) - 详细描述服务器网关、消息路由和各服务端分离的设计方案以及完整的通信机制
...
- **位置**：`src/network/`
- **核心技术**：Boost::asio + Boost::beast
- **功能**：
  - 处理底层网络通信，支持TCP、WebSocket和HTTP协议
  - 基于事件驱动的IO模型，高效处理大量并发连接
  - 统一的连接管理架构，通过Connection基类实现多协议统一接口
  - 完善的数据传输和缓冲区管理机制
  - 完整的连接生命周期管理（创建、启动、关闭、销毁）
  - 实时连接状态监控和统计信息收集
  - 支持连接上下文扩展，便于存储自定义数据
  - 线程安全的设计，支持多线程并发操作
  - 自动清理空闲连接，优化资源使用
  - 支持连接事件通知，便于上层模块处理连接状态变化
...
### 3.2 网络组件
- **Connection**：抽象连接基类，定义统一连接接口，所有具体连接类型的父类
  - 提供统一的连接生命周期管理接口（start、close、forceClose）
  - 支持统一的数据传输接口（send方法重载）
  - 内置连接统计信息（字节数、消息数、连接时间等）
  - 支持连接上下文扩展，可存储自定义数据
  - 线程安全的设计，使用shared_mutex保护状态

- **ConnectionManager**：统一连接管理器，管理所有类型的连接
  - 支持添加、移除、查询连接
  - 按连接类型、状态进行分组管理
  - 自动清理空闲连接
  - 提供全局连接统计信息
...
- **TcpServer**：基于Boost::asio的TCP服务器实现
  - 异步接受新连接
  - 创建TcpConnection对象管理连接生命周期
  - 支持IPv4/IPv6
  - 可配置监听地址和端口

- **TcpConnection**：TCP连接的具体实现
  - 基于Boost::asio::ip::tcp::socket
  - 异步读写操作
  - 线程安全的发送队列
  - 自动缓冲区管理

- **WebSocketServer**：基于Boost::beast的WebSocket服务器实现
  - 异步接受新连接并进行WebSocket握手
  - 创建WebSocketConnection对象管理连接生命周期
  - 支持标准WebSocket协议
  - 自动处理WebSocket帧

- **WebSocketConnection**：WebSocket连接的具体实现
  - 基于boost::beast::websocket::stream
  - 支持文本和二进制消息
  - 内置消息队列，线程安全
  - 异步读写操作
...
- **HttpConnection**：HTTP连接的具体实现
  - 基于Boost::beast::http
  - 自动解析HTTP请求
  - 支持异步处理请求和响应
  - 内置超时处理机制

- **IdGenerator**：全局唯一ID生成器，用于生成连接ID等
  - 线程安全的ID生成
  - 支持多种ID类型（连接ID、用户ID、消息ID等）
  - 基于时间戳和进程ID的唯一ID
  - 支持UUID和短ID生成
...
### 4.1 客户端连接流程
1. 客户端发起TCP/WebSocket/HTTP连接请求
2. 对应协议的服务器（TcpServer/WebSocketServer/HttpServer）接收连接
3. 调用IdGenerator生成唯一连接ID
4. 创建对应类型的Connection对象（TcpConnection/WebSocketConnection/HttpConnection）
5. 将Connection对象添加到ConnectionManager统一管理
