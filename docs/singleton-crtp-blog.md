# 深入理解 C++ 单例模式：从 CRTP 到 `std::call_once`

> 以 IMServer 项目中的 `Singleton<T>` 模板为例，拆解奇异递归模板（CRTP）与线程安全懒加载的实现细节。
...
4. 但 `Singleton<RedisMgr>` 并不是 `RedisMgr` 的友元——
...
## 五、核心机制 III：`std::call_once` 实现线程安全懒加载
...
*博客基于 IMServer 项目（2026/07/06 版本）实际代码撰写。如有讨论，欢迎评论区交流。*
